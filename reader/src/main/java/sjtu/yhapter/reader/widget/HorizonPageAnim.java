...
public abstract class HorizonPageAnim extends PageAnimation{
    ...
    public HorizonPageAnim(Context context, int w, int h) {
        this(context, w, h, 0, 0);
    }

    public HorizonPageAnim(Context context, int w, int h, int marginWidth, int marginHeight) {
    ...
    public boolean dispatchTouchEvent(MotionEvent event) {
        ...
        switch (event.getAction()){
            ...
            case MotionEvent.ACTION_MOVE:
                final int slop = ViewConfiguration.get(App.getInstance()).getScaledTouchSlop();
                //判断是否移动了
                if (!isMove) {
                ...
                if (isMove){
                    ...
                    if (mMoveX == 0 && mMoveY ==0) {
                        ...
                        if (x - mStartX > 0){
                            //上一页的参数配置
                            isNext = false;
                            boolean hasPrev = mListener.hasPrePage();
                            ...
                            isNext = true;
                            //判断是否下一页存在
                            boolean hasNext = mListener.hasNextPage();
                            //如果存在设置动画方向
                    ...
                    mMoveY = y;
                    isRunning = true;
                    mListener.requestInvalidate(false);
                }
                break;
